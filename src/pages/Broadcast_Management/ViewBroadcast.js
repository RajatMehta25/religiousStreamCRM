    ...
    if (!state) {
      try {
        const { data } = await axios.post("/common/sendEmail", {
          to: values.email,
          subject: "Welcome to Live Adhan",
          message: `Hello,<br/>
                   <p>Thanks for choosing Live Adhan, your mosque ${values.name} has been been successfully created now you can login in Broadcast app with given password.<br/>
      ...
      try {
        const { data } = await axios.post("/common/sendEmail", {
          to: values.email,
          subject: "Account Updation Live Adhan",
          message: `Hello,<br/>
            ...
            <div className={classes.paperPaddingRightLeft}>
              <div className="py-4">
                <Paper elevation={0} className={classNames(classes.paperHeading, classes.addNewCategory)}>
                  ...
                  <div>
                    <Tooltip title={<span style={{ color: "white", fontSize: "16px" }}>QR</span>} arrow>
                      <IconButton
                        className=""
                        style={{ backgroundColor: "#0E3F37", color: "#fff", marginLeft: "5px" }}
                        onClick={() => {
                          props.history.push({
                            pathname: "/adminPanel/Qr",
                            state: state,
                          });
                        }}
                      >
                ...
                <Paper
                  ...
                  elevation={0}
                >
                  <div style={{ margin: "2rem 0 2rem 0", pointerEvents: !state ? "" : "none" }}>
                    <Formik
                      ...
                      initialValues={{
                        ...
                        startTime:
                          get(state, "jumma.start", null) !== "Invalid date"
                            ? moment(get(state, "jumma.start", null), ["hh:mm A"]).local().format("HH:mm")
                            : null,
                        endTime:
                          get(state, "jumma.end", null) !== "Invalid date"
